...
from services.languages import translate, extract_language_and_update_if_not_present
...
def select_staff(update, context):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f'❕{translate("call_waiter", language=lang)}',
...
def call_cashier(update, context):
    ...
    bot.send_message(os.environ["WORKERS_CHANNEL"], f"""Вызов официанта\nот {first_name} {last_name} {username}""")
    bot.edit_message_text(
        chat_id=query.message.chat_id,
        message_id=query.message.message_id,
        text=translate("staff_request_sent", lang)
